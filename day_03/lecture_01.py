# -*- coding: utf-8 -*-
"""
Created on Fri Mar 18 15:12:40 2022
...
# 설명변수(X) : 특정 종속 변수를 유추하기 위해서 정의된 데이터 셋
...
# 설명변수 X에 대한 데이터 탐색
# - 데이터의 개수, 컬럼의 개수
# - 각 컬럼 데이터에 대한 결측 데이터의 존재 유무(Non-Null count)
# - 각 컬럼 데이터의 데이터 타입
#  (반드시 수치형의 데이터만 머신러닝에 활용할 수 있음)
print(X.info())

# pandas 라이브러리 옵션을 설정
# -출력 컬럼의 개수 제어(기본 5개값 -> 30개 값으로 해줌)
...
# 데이터를 구성하는 컬러들의 스케일 부분을 중점적으로 체크
# (스케일 : 특정 컬럼 값의 범위)
...
# 범주형 값의 확인 및 개수 체크(카테고리가 몇갠지, 카테고리의 갯수는 몇갠지)
...
# 위와 같은 전처리 코드의 경우 데이터의 전체 모습을 미리 확인한 후
...
# 3. 데이터의 분할
# - 학습 데이터(70%, 80%), 테스트 데이터(30%, 20%) (머신러닝의 case)
# - 학습 데이터(70%), 검증 데이터(10%), 테스트 데이터(20%) (딥러닝의 case)
#   (딥러닝의 경우 부분 배치 학습을 수행하여 점진적으로 학습량을
#    늘려나가는 경우가 많음, 중간 점검의 의미로 검증 데이터를 활용)
...
#            (y 데이터가 범주형인 경우)
#            각 범주형 값의 비율을 유지하면서 데이터를 분석하는 열할
...
# 전처리 과정의 데이터 학습은 학습데이터를 기준으로 수행
# - 학습 데이터의 최소 / 최대값을 기준으로 스케일링 수행 준비

scaler.fit(X_train)

# - 학습 데이터를 스케일 처리 수행
X_train = scaler.transform(X_train)
# - 테스트 데이터는 학습 데이터를 기준으로 변환만 수행
#   (극단적으로 - 값이나 1이상의 값이 있을 수 있음 그래서 이상적이지 않은 경우에도 예측가능한...)
...
# - n_jobs = -1 하면 사용가능한 CPU를 모두 활용하여 계산함 (계산 빨라짐)
...
# - 학습의 완료된 머신러닝 모델 객체의 평가
# - score 메소드 사용
# - 모델객체.score(X,y)
#   (입력된 X를 사용하여 예측을 수행하고 예측된 값을 입력된 y와 비교하여 
#    평가 결과를 반환)
...
#   예측의 결과를 수용하는 방법을 사용
proba=model.predict_proba(X_test[-7:])
