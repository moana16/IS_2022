...
#      (실제 정답과 실제 정답의 평균 값 차이의 제곱 값 합계))
...
# R2(결정계수)의 값이 1인 경우 
# - 머신러닝 모델이 예측한 값이 실제 정답과 완벽하게 일치하는 경우
# - 머신러닝 모델의 학습이 잘 진행됨을 확인 (과대적합의 의심...)

# R2(결정계수)의 값이 0보다 작은 경우
# - 머신러닝 모델이 예측하는 값이 정답들의 평균 조차 예측하지 못하는 경우
# - 머신러닝 모델의 학습이 부족함 (과소적합)
...
pred=3.25 * model.coef_[0] + 39.0 * model.coef[1] + \
     4.503205 * model.coef_[3] + 1.073718 * model.coef_[4] +\
     1109.0 * model.coef_[4] + 1.777244 * model.coef_[5] + \
     34.06 * model.coef_[6] + -118.35 * model.coef_[7] + \
     model.intercept_
...
# - R2(결정계수) : -1 ~ 1 사이의 값을 가짐
#                  데이터에 관계없이 동일한 결과의 범위를 사용하여 모델을 평가

# - 평균절대오차 : 실제 정답과 모델이 예측한 값의 차이를 절대값으로 평균
#                 (머신러닝 모델이 예측한 값의 신뢰 범위)

# - 평균절대오차비율 : 실제 정답과 모델이 예측한 값의 비율 차이를 절대값으로 평균낸 비율
